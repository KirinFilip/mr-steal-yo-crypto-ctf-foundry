...
contract Testing is Test {
    ...
    function setUp() public {
        ...
        erc1820Registry = IERC1820Registry(deployCode("out/ERC1820Registry.sol/ERC1820Registry.json"));
        ...
        address(usdc), address(wbtc), 2_000_000e18, 2_000e18, 0, 0, admin, block.timestamp);

        usdcBtcPair = IUniswapV2Pair(uniFactory.getPair(address(usdc), address(wbtc)));

        // deploying core contracts
        vm.prank(admin);
        moneyMarket = IMoneyMarket(deployCode("out/MoneyMarket.sol/MoneyMarket.json"));

        vm.prank(admin);
        moneyMarket._supportMarket(address(wbtc));

        vm.prank(adminUser);
        wbtc.approve(address(moneyMarket), type(uint256).max);
        vm.prank(adminUser);
