...
contract Testing is Test {
    ...
    function setUp() public {
        ...
        vm.prank(admin);
        IERC20(address(weth)).approve(address(uniRouter), type(uint256).max);
        ...
        address(usdc), address(weth), 1_000_000e18, 500e18, 0, 0, admin, block.timestamp);
        ...
        vm.prank(admin);
        optionsContract =
            new CallOptions(address(usdc), address(weth), address(usdcEthPair), address(uniFactory), address(uniRouter));

        // --interacting with core contracts
        vm.prank(adminUser2);
        IERC20(address(weth)).approve(address(optionsContract), type(uint256).max);
        ...
        vm.prank(adminUser);
        usdc.approve(address(optionsContract), type(uint256).max);
        vm.prank(adminUser);
