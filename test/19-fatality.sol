...
contract Testing is Test {
    ...
    function setUp() public {
        ...
        bnb.approve(address(uniRouter), type(uint256).max);
        vm.stopPrank();
        ...
        address(usdc), address(bnb), 900_000e18, 3_000e18, 0, 0, admin, block.timestamp);

        vm.prank(admin);
        uniRouter.addLiquidity( // DAI-BNB pair
        address(dai), address(bnb), 900_000e18, 3_000e18, 0, 0, admin, block.timestamp);
        ...
        vm.prank(admin);
        bunnyMinter =
            new BunnyMinter(address(zapBSC), address(uniRouter), address(bnb), address(bunny), address(bnbBunnyPair));
