...
contract Revest is IRevest, AccessControlEnumerable, RevestAccessControl, RevestReentrancyGuard {
    ...
    function mintAddressLock(
...

        {
            ...
            uint256 lockId = getLockManager().createLock(fnftId, addressLock);
        }
    ...
    function withdrawFNFT(uint256 fnftId, uint256 quantity) external override revestNonReentrant(fnftId) {
    ...
    function unlockFNFT(uint256 fnftId) external override {
        ...
        returns (uint256)
    {
        ...
        {
            uint256 supply = IFNFTHandler(handler).getSupply(fnftId);

            uint256 balance = IFNFTHandler(handler).getBalance(_msgSender(), fnftId);
        ...
        uint256 lockId = ILockManager(lockHandler).fnftIdToLockId(fnftId);
    ...
    function doMint(
        ...
        bool isSingular;
        uint256 totalQuantity = quantities[0];
        {
            uint256 rec = recipients.length;
            uint256 quant = quantities.length;
            require(rec == quant, "recipients and quantities arrays must match");
            // Calculate total quantity
            isSingular = rec == 1;
            if (!isSingular) {
                for (uint256 i = 1; i < quant; i++) {
